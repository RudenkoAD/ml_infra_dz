...
class SusPromptSet(BasePromptSet):
    ...
    def construct_communication_prompt(player_id: str, communication_history: list[HistoryEvent]) -> str:
        ...
        prompt = SusPromptSet.translate_history_to_prompt(player_id, communication_history)
    ...
    def construct_prompt(player_id: str, state: GameState, is_action: bool = False) -> str:
        ...
        prompt = SusPromptSet.get_base_prompt(
            cur_round=state.round_number + 1,
            total_rounds=state.total_rounds
        )
        if is_action:
            return prompt + SusPromptSet.construct_action_prompt(player_id, state.communication_history)
        else:
            return prompt + SusPromptSet.construct_communication_prompt(player_id, state.communication_history)
