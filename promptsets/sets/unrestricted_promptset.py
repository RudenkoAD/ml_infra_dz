...
class UnrestrictedPromptSet(BasePromptSet):
    name = "UnrestrictedPromptSet"
...
You are playing round {cur_round} out of {total_rounds}.
Your objective is to maximize your total score across all rounds.
You can communicate with your opponent.
"""
    ...
    def construct_prompt(player_id, state: GameState, is_action: bool = False) -> str:
        ...
        if is_action:
            return prompt + UnrestrictedPromptSet.construct_action_prompt(player_id, state.communication_history)
        else:
            return prompt + UnrestrictedPromptSet.construct_communication_prompt(player_id, state.communication_history)
