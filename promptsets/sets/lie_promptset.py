...
class LiePromptSet(BasePromptSet):
    ...
    def construct_communication_prompt(player_id, communication_history: list[HistoryEvent]) -> str:
        ...
        prompt = LiePromptSet.translate_history_to_prompt(player_id, communication_history)
    ...
    def construct_prompt(player_id, state: GameState, is_action: bool = False) -> str:
        ...
        prompt = LiePromptSet.get_base_prompt(
            cur_round=state.round_number + 1,
            total_rounds=state.total_rounds
        )
        if is_action:
            return prompt + LiePromptSet.construct_action_prompt(player_id, state.communication_history)
        else:
            return prompt + LiePromptSet.construct_communication_prompt(player_id, state.communication_history)
