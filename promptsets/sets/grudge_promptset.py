...
class GrudgePromptSet(BasePromptSet):
    name = "GrudgePromptSet"
...
2. If the opponent has stolen from you, trust is permanently broken. You must STEAL for all remaining rounds of the game.
Your communication should reflect your strategy. Be cooperative initially, warn your opponent about the consequences of betrayal, and be firm and final if it happens.
"""
    ...
    def construct_communication_prompt(player_id, communication_history: list[HistoryEvent]) -> str:
        ...
        prompt = GrudgePromptSet.translate_history_to_prompt(player_id, communication_history)
    ...
    def construct_prompt(player_id, state: GameState, is_action: bool = False) -> str:
        ...
        prompt = GrudgePromptSet.get_base_prompt(
            cur_round=state.round_number + 1,
            total_rounds=state.total_rounds
        )
        if is_action:
            return prompt + GrudgePromptSet.construct_action_prompt(player_id, state.communication_history)
        else:
            return prompt + GrudgePromptSet.construct_communication_prompt(player_id, state.communication_history)
